...
export default function MarketingPush() {
  ...
  const [form, setForm] = useState({ name: "", audience: "App Users", title: "", body: "" });
  ...
  function load(overrides = {}) {
    api(`/admin/marketing-push?${qs(overrides)}`)
      .then((d) => { setData(d); setError(""); })
      .catch((e) => setError(e.message || "Could not load push notifications."));
  }
  ...
  function saveForm(e) {
    e.preventDefault();
    if (!form.name.trim() || !form.title.trim()) return setToast("Name and title are required");
    setFormOpen(false);
    setToast(`Push “${form.name}” saved as draft`);
    setForm({ name: "", audience: "App Users", title: "", body: "" });
  }

  if (!data) {
    return (
      <div className="mktpg-page">
        <nav className="crumbs"><Link to="/">Dashboard</Link><span>›</span><Link to="/marketing">Marketing</Link><span>›</span><strong>Push Notifications</strong></nav>
        {error ? <p className="error">{error}</p> : <p className="muted">Loading push notifications…</p>}
      </div>
    );
  }
  ...
  return (
    <div className="mktpg-page">
      ...
      <div className="prod-head">
        <div>
          <h1><span className="prod-title-icon solid"><Icon name="bell" size={16} /></span> Push Notifications</h1>
          <p>Engage app users with timely push alerts and offers.</p>
        </div>
        <div className="prod-actions">
          <button className="btn btn-ghost btn-small" type="button" onClick={() => navigate("/marketing")}>Back to Overview</button>
          <button className="btn btn-purple btn-small" type="button" onClick={() => setFormOpen(true)}><Icon name="plus" size={14} /> New Push</button>
        </div>
      </div>
      ...
      <section className="pts-stats five mktpg-kpis">
        <article className="prod-stat cat-stat"><div><div className="muted">Pushes Sent</div><div className="prod-stat-n purple">{fmtNum(stats.sent)}</div><div className="cat-stat-hint up">↑ {stats.sentDelta}%</div></div><div className="prod-stat-icon purple"><Icon name="bell" size={16} /></div></article>
        <article className="prod-stat cat-stat"><div><div className="muted">Open Rate</div><div className="prod-stat-n green">{fmtNum(stats.openRate, 1)}%</div><div className="cat-stat-hint up">↑ {stats.openDelta}%</div></div><div className="prod-stat-icon green"><Icon name="eye" size={16} /></div></article>
        <article className="prod-stat cat-stat"><div><div className="muted">Click Through</div><div className="prod-stat-n blue">{fmtNum(stats.ctr, 1)}%</div><div className="cat-stat-hint up">↑ {stats.ctrDelta}%</div></div><div className="prod-stat-icon blue"><Icon name="trend" size={16} /></div></article>
        <article className="prod-stat cat-stat"><div><div className="muted">Registered Devices</div><div className="prod-stat-n orange">{fmtNum(stats.devices)}</div><div className="cat-stat-hint up">↑ {stats.devicesDelta}%</div></div><div className="prod-stat-icon orange"><Icon name="phone" size={16} /></div></article>
      ...
      <section className="card prod-filters">
        <form className="prod-filter-row" onSubmit={(e) => { e.preventDefault(); setPage(1); load({ q, page: 1 }); }}>
          <div className="prod-search"><Icon name="search" size={16} /><input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search notifications..." /></div>
      ...
      <section className="card prod-table-wrap mktpg-table-card">
        <table className="table prod-table">
          <thead>
            <tr><th>#</th><th>Notification</th><th>Status</th><th>Audience</th><th>Platform</th><th>Sent</th><th>Open Rate</th><th>CTR</th><th>Scheduled</th><th>Actions</th></tr>
          </thead>
          <tbody>
            {rows.map((r) => (
              <tr key={r.id}>
                ...
                <td>
                  <div className="prod-row-acts" onClick={(e) => e.stopPropagation()}>
                    ...
                    <DeliveryRowMenu id={r.id} menu={menu} setMenu={setMenu} up={r.n >= rows.length - 1}>
                      ...
                      <button type="button" onClick={(e) => { e.stopPropagation(); setMenu(null); setToast(r.status === "scheduled" ? `Cancelled “${r.name}”` : `Queued “${r.name}”`); }}>{r.status === "scheduled" ? "Cancel" : "Send now"}</button>
                    </DeliveryRowMenu>
                  </div>
                </td>
              </tr>
            ))}
            {rows.length === 0 && <tr><td colSpan="10" className="muted">No notifications found.</td></tr>}
          </tbody>
        </table>
        <footer className="prod-pager">
          <span>Showing {fromN} to {toN} of {fmtNum(total)} notifications</span>
        ...
        <div className="prod-modal" onClick={() => setFormOpen(false)}>
          <form className="card prod-modal-card" onClick={(e) => e.stopPropagation()} onSubmit={saveForm}>
            <div className="ord-drawer-head">
              <div><h2>New Push Notification</h2><p className="muted">Compose a push for iOS and Android devices.</p></div>
            ...
            <div className="form-grid">
              ...
              <label className="full">Title<input value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} required /></label>
              <label className="full">Body<textarea rows={3} value={form.body} onChange={(e) => setForm((f) => ({ ...f, body: e.target.value }))} /></label>
            </div>
      ...
      {viewing && (
        <DeliveryDetailModal title={viewing.name} subtitle={viewing.platform} statusNode={<span className={`st-pill ${statusCls(viewing.status)}`}>{viewing.statusLabel}</span>} onClose={() => setViewing(null)} actions={<button className="btn btn-purple btn-small" type="button" onClick={() => { setViewing(null); setToast(`Queued “${viewing.name}”`); }}>Send now</button>}>
          <DetailMeta rows={[
